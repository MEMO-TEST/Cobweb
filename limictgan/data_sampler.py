...
class DataSampler(object):
    ...
    def generate_cond_from_data(self, data_rows):
        """
        根据采样到的数据行生成条件向量 `cond`。

        Args:
            data_rows (numpy.ndarray):
                采样到的原始数据行，形状为 `(n_rows, n_features)`。

        Returns:
            cond (numpy.ndarray):
                生成的条件向量，形状为 `(n_rows, self._n_categories)`。
            col (list[int]):
                每个数据行对应的离散列索引。
            opt (list[int]):
                每个数据行对应的类别索引（在离散列中）。
        ...
        cond = np.zeros((n_rows, self._n_categories), dtype="float32")
        col = []
        opt = []
        ...
        for i, row in enumerate(data_rows):
            # 遍历所有离散列
            for col_id, rid_by_cat in enumerate(self._rid_by_cat_cols):
                # 遍历该列中所有类别，寻找匹配的类别
                for cat_id, rows_in_category in enumerate(rid_by_cat):
                    # 如果行索引匹配，记录列和类别信息
                    if i in rows_in_category:
                        col.append(col_id)  # 列索引
                        opt.append(cat_id)  # 类别索引
                        # 将类别对应的全局索引设置为 1
                        global_category_idx = self._discrete_column_cond_st[col_id] + cat_id
                        cond[i, global_category_idx] = 1
                        break
